...
## 📅 2026-02-12：第六阶段：HBase 架构集成与环境部署

**2026/2/12 11:30**
*   **事项**: HBase 生产级集群环境构建
*   **核心配置**:
    1.  **[hbase-env.md](file:///e:/a_VibeCoding/EcoPulse/CoreCode_Enterprise/hbase-env.md)**: 配置了 `JAVA_HOME` 并明确了使用外部 Zookeeper (Mode: False) 以避免资源冲突。
    2.  **[hbase-site.md](file:///e:/a_VibeCoding/EcoPulse/CoreCode_Enterprise/hbase-site.md)**: 实现了 HBase 2.4.9 与 Hadoop 3.3.0 (hdfs://hadoop1:9000) 的深度对接，并开启了 Thrift Server 代理以支持 Python 生态接入。
    3.  **[regionservers.md](file:///e:/a_VibeCoding/EcoPulse/CoreCode_Enterprise/regionservers.md)**: 优化了节点拓扑，将 Master 节点纳入计算资源池 (3节点全量部署)，提升了 50% 的并行处理能力。
*   **验证状态**:
    *   ✅ **HMaster/HRegionServer**: 启动正常。
    *   ✅ **ThriftServer**: 端口 9090 监听正常。
    *   ✅ **Python SDK**: 具备了通过 `happybase` 进行连接的基础能力。


12.19
1. Hive 建表脚本 ( CoreCode_Enterprise/hive_ddl.sql )
- 定义了 ecop.ods_user_behavior 外部表，直接映射到 HDFS 上的 CSV 文件。
- 预创建了 DWD 和 ADS 层的所有表结构，为后续 ETL 做好准备。 2. 数据接入脚本 ( CoreCode_Enterprise/ingest_to_hdfs.py )
- 功能 : 将 data/row/ 下的 7 个月 CSV 文件自动上传到 HDFS 对应分区目录。
- 自动化 : 上传后自动执行 MSCK REPAIR TABLE ，无需手动添加分区。
- 幂等性 : 脚本包含文件存在检查，避免重复上传。


**2026/2/12 12:40**
*   **事项**: ODS 层接入验证 (Smoke Test)
*   **验证策略**: 优先选取 `2019-Oct` (5.4GB) 单月数据进行端到端跑通。
*   **执行脚本**:
    1.  [execute_ddl.py](file:///e:/a_VibeCoding/EcoPulse/CoreCode_Enterprise/execute_ddl.py) ([说明文档](file:///e:/a_VibeCoding/EcoPulse/CoreCode_Enterprise/execute_ddl.md)): 成功在 Spark 内部 Catalog 中构建了完整的数仓分层结构。
    2.  [verify_hdfs_upload.py](file:///e:/a_VibeCoding/EcoPulse/CoreCode_Enterprise/verify_hdfs_upload.py) ([说明文档](file:///e:/a_VibeCoding/EcoPulse/CoreCode_Enterprise/verify_hdfs_upload.md)): 验证 HDFS 文件完整性，确认 10 月份数据大小为 **5406.01 MB**。
    3.  [verify_data_read.py](file:///e:/a_VibeCoding/EcoPulse/CoreCode_Enterprise/verify_data_read.py) ([说明文档](file:///e:/a_VibeCoding/EcoPulse/CoreCode_Enterprise/verify_data_read.md)): 验证数据可读性，成功读取并统计出 **42,448,764 条** 记录。
*   **里程碑**: **Phase E2 小规模验证通过**。HDFS 写入链路、权限配置 (root) 及 PySpark 读取链路均已打通，具备了执行全量 54GB 数据接入的条件。
...
*   **诊断**: 
    *   **DFS Used%**: **99.70%** (已满)
    *   **DFS Remaining**: **244.15 MB** (已不足以上传剩余的 15GB 数据)
    *   **原因**: 两个 DataNode (hadoop2, hadoop3) 的磁盘空间均已耗尽。`hadoop2` 剩余 165MB，`hadoop3` 剩余 78MB。
*   **阻断影响**: 全量数据接入任务 (Phase E2) 被迫中断。2020-Jan 和 2020-Mar 数据无法写入。
*   **应对方案**: 需立即执行磁盘清理或扩容。建议优先清理 `/export/data/` 下的无用数据或调整副本策略 (Replication Factor = 1)。
...
**2026/2/12 13:45**
*   **事项**: HDFS 空间救援
*   **操作**: 执行副本数降级策略 (`hdfs dfs -setrep -R -w 1 /ecop`)。
*   **结果**:    *   **DFS Remaining**: 恢复至 **39.20 GB** (足够上传剩余数据)。
    *   **状态异常**: 数据分布极度不均衡。
        *   `hadoop2`: Used 90.13% (仍旧爆满)。
        *   `hadoop3`: Used 0.01% (几乎全空)。
    *   **分析**: 降级操作导致 hadoop3 上的副本被删除，而 hadoop2 上的副本被保留。
*   **下一步**: 重新运行全量上传脚本。HDFS 客户端应能感知 hadoop2 空间不足，自动将新数据 (2020-Jan, 2020-Mar) 路由写入到空闲的 `hadoop3` 节点。


**2026/2/12 13:50**
*   **事项**: Phase E2 (ODS 全量接入) 完成
*   **结果**: 全量 7 个月 CSV 数据 (54GB) 已成功上传至 HDFS。
*   **细节**:
    *   **空间救援**: 通过副本数降级 (`setrep 1`) 成功释放了 40GB 空间。
    *   **负载均衡**: HDFS 客户端自动将最后两个月的数据 (2020-Jan, 2020-Mar) 路由写入到了空闲的 `hadoop3` 节点，解决了 `hadoop2` 爆满的问题。
    *   **完整性**: 
        *   2019: Oct, Nov, Dec (✅ All Done)
        *   2020: Jan, Feb, Mar, Apr (✅ All Done)
*   **下一步**: 需要修复 Hive 元数据 (由于之前禁用了 Metastore 连接)，建议在 Hive CLI 中执行 `MSCK REPAIR TABLE ecop.ods_user_behavior` 以识别新分区。


**2026/2/12 14:00**
*   **事项**: Phase E3 (DWD/ADS 重构) 启动
*   **架构升级**: 
    1.  **公共模块**: 创建 `common/spark_config.py`，统一了 SparkSession 的构建逻辑（含 HDFS/Hive/HBase 配置及环境变量处理）。
        1.  **公共模块**: 创建 `common/spark_config.py` ([说明文档](file:///e:/a_VibeCoding/EcoPulse/common/spark_config.md))，统一了 SparkSession 的构建逻辑（含 HDFS/Hive/HBase 配置及环境变量处理）。
    2.  **ETL 重构**: 开发 [etl_dwd_enterprise.py](file:///e:/a_VibeCoding/EcoPulse/CoreCode_Enterprise/etl_dwd_enterprise.py) ([说明文档](file:///e:/a_VibeCoding/EcoPulse/CoreCode_Enterprise/etl_dwd_enterprise.md))。
*   **核心特性**:
    *   **Bot 过滤**: 引入 Session 频次检测，剔除 >500 events 的异常会话。
    *   **价格审计**: 增加 `price_is_illegal` 标记位，而非直接丢弃数据。
    *   **动态分区**: 采用 Hive 动态分区机制写入 DWD 层。
